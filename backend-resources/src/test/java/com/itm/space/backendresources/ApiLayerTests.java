...
@WithMockUser(username = "testUser", roles = "MODERATOR") // Устанавливаем пользователя и роль для всех тестов
public class ApiLayerTests {
    ...
    public void testCreateUser() throws Exception {
        ...
        mockMvc.perform(post("/api/users")
                        ...
                        .content(userJson))
                .andExpect(status().isOk()); // Ожидаем статус 200 (или любой другой успешный статус)

        // Проверяем, что метод createUser() был вызван
    ...
    public void testGetUserById() throws Exception {
        ...
        mockMvc.perform(get("/api/users/{id}", userId)
                ...
                .andExpect(jsonPath("firstName").value("testUsername")); // Проверяем, что имя пользователя в ответе соответствует
    ...
    public void testHello() throws Exception {
        ...
        mockMvc.perform(get("/api/users/hello"))
                .andExpect(status().isOk()) // Ожидаем статус 200
                .andExpect(content().string("testUser")); // Проверяем, что в ответе имя пользователя соответствует "testUser"
    }
}
